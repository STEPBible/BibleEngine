...
function getDictionaryEntries(
  ...
  Object.entries(lexiconEntries).forEach(([strong, entry]) => {
    ...
    const greekTransliteration: string = (<any>entry)[
      LexiconEntryType.GREEK_TRANSLITERATION
    ];
    const hebrewTransliteration: string = (<any>entry)[
      LexiconEntryType.HEBREW_TRANSLITERATION
    ];
    const gloss: string = (<any>entry)[LexiconEntryType.GLOSS];

    const hebrewDefinitions: string[] =
      (<any>entry)[LexiconEntryType.HEBREW_DEFINITION] || [];

    const greekDefinition: string = (<any>entry)[
      LexiconEntryType.GREEK_DEFINITION
    ];
    const shortGreekDefinition: string = (<any>entry)[
    ...
    if (
      dictionary === LexiconEntryType.GREEK_DEFINITION &&
      shortGreekDefinition
    ) {
