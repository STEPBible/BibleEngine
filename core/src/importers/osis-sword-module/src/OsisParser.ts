...
export function getBibleEngineInputFromXML(
  bookXML: ChapterXML[]
): IBibleContentSectionForInput[] {
...
function parseTextNode(text: string, context: ParserContext) {
  ...
  if (isFootnote(context.currentNoteNode)) {
    context.currentNoteNum =
      Number(context.currentNoteNode!.attributes.n) ||
  ...
  switch (tagName) {
    ...
    case OsisXmlTag.QUOTE: {
      const isClosingQuotationMark =
        node && node.isSelfClosing && node.attributes.marker;
...
function getCrossReference(context: ParserContext): IBibleCrossReference {
  ...
  let {
    ...
    versionChapterNum,
    versionVerseNum
  } = getOsisReferenceEntities(osisRef);
...
function getIndentGroup(
  ...
  if (node!.attributes.level! >= Indentation.LARGE) {
    ...
    throw new Error(
      `Unrecognized indentation level: ${JSON.stringify(node.attributes.level)}`
    );
  }
}
