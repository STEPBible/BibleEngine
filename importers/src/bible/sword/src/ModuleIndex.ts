...
export default class ModuleIndex {
    ...
    constructor(files: object, config: ModuleConfig) {
        ...
        const rawPosOT = this.getChapterVersePositions(
            oldTestament.chapterVersePositions,
            bookPosOT,
            'ot',
            versification
        );
        ...
        const rawPosNT = this.getChapterVersePositions(
            newTestament.chapterVersePositions,
            bookPosNT,
            'nt',
            versification
        );
    ...
    getChapterVersePositions(
        inBuf: Uint8Array,
        inBookPositions: { length: number; startPos: number; unused: number }[],
        inTestament: string,
        inV11n: string
    ) {
        ...
        const booksEnd =
            inTestament === 'ot'
                ? VerseScheme.getBooksInOT(inV11n)
                : VerseScheme.getBooksInOT(inV11n) + VerseScheme.getBooksInNT(inV11n);
            ...
            for (let c = 0; c < bookData.maxChapter; c++) {
                ...
                for (let v = 0; v < verseMax; v++) {
                    ...
                    if (length !== 0) {
                        ...
                        osis = `${bookData.abbrev}.${parseInt(String(c + 1), 10)}.${parseInt(
                            String(v + 1),
                            10
                        )}`;
