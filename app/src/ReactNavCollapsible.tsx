...
export const setExpoStatusBarHeight = height => {
  if (Platform.OS === 'android' && global.Expo) androidStatusBarHeight = height
}
...
export const setSafeBounceHeight = height => {
...
const CollapsibleExtraHeader = props => {
  ...
  const translateY = animatedDiffClampY
    ? getTranslateY(animatedDiffClampY, height)
    : 0
  const defaultOpacity =
    collapsibleBackgroundStyle.opacity != null
      ? collapsibleBackgroundStyle.opacity
      : 1
...
class _CollapsibleHeaderBackView extends Component {
  ...
  render() {
    ...
    if (
      ...
      !navigation.state.params.animatedDiffClampY ||
    ...
    return (
      <Animated.View
        style={{
          ...
          position: 'absolute',
          width: '100%',
          height: navigationHeight - STATUS_BAR_HEIGHT,
        }}
      />
    )
  }
...
const collapsibleNavigationOptions = (
  configOptions,
  userOptions,
  navigation
) => {
  ...
  const {
    collapsibleTranslateY,
    collapsibleTranslateOpacity,
  } = navigationParams
  const headerHeight =
  ...
  if (navigationParams.headerHeight !== headerHeight) {
    const animatedDiffClampY = Animated.diffClamp(
      navigationParams.animatedYSum,
      0,
      getSafeBounceHeight() + headerHeight
    )
    navigation.setParams({
      ...
      collapsibleTranslateProgress: getTranslateProgress(
        animatedDiffClampY,
        headerHeight
      ),
    })
  ...
  const collapsibleOptions = {
    ...
    headerStyle: {
      ...
      opacity: Platform.select({
        ios: collapsibleTranslateOpacity || 1,
        android: global.Expo ? collapsibleTranslateOpacity || 1 : 1,
        web: 1,
      }),
      height: headerHeight,
    },
...
const getCollapsibleHeaderHeight = navigationParams =>
  (navigationParams && navigationParams.headerHeight) || 0
...
export const withCollapsible = (
  ...
  const collapsibleType = collapsibleParams.collapsibleComponent
    ? CollapsibleType.extraHeader
    : CollapsibleType.defaultHeader
  ...
  class _withCollapsible extends Component {
    constructor(props) {
      ...
      this.onScroll = this.animatedY.map(animatedY =>
        Animated.event([{ nativeEvent: { contentOffset: { y: animatedY } } }], {
      ...
      switch (collapsibleType) {
        ...
        case CollapsibleType.extraHeader:
          {
            const headerHeight =
              ((collapsibleParams.collapsibleBackgroundStyle &&
                collapsibleParams.collapsibleBackgroundStyle.height) ||
                0) -
              ((collapsibleParams.collapsibleBackgroundStyle &&
                collapsibleParams.collapsibleBackgroundStyle.paddingBottom) ||
                0)
            const animatedDiffClampY = Animated.diffClamp(
              this.animatedYSum,
              0,
            ...
            this.props.navigation.setParams({
              animatedDiffClampY,
              collapsibleTranslateY: getTranslateY(
                animatedDiffClampY,
                headerHeight,
                headerHeight
              ),
              collapsibleTranslateOpacity: getOpacity(
                animatedDiffClampY,
                headerHeight
              ),
              collapsibleTranslateProgress: getTranslateProgress(
    ...
    render() {
        ...
        case CollapsibleType.defaultHeader:
          {
            ...
            if (collapsibleHeaderHeight) {
              ...
              paddingHeight =
                collapsibleHeaderHeight + getStatusBarHeight(isLandscape)
            }
          }
          break
        case CollapsibleType.extraHeader:
      ...
      const props = {
        ...
        collapsible: {
          ...
          translateY:
            params.collapsibleTranslateY || new Animated.Value(paddingHeight),
          translateOpacity:
            params.collapsibleTranslateOpacity || new Animated.Value(1),
          translateProgress:
            params.collapsibleTranslateProgress || new Animated.Value(0),
        },
      }
        ...
        case CollapsibleType.extraHeader:
          collapsibleComponent = (
            <CollapsibleExtraHeader
              navigation={navigation}
              style={collapsibleParams.collapsibleBackgroundStyle}
            >
    ...
    case CollapsibleType.defaultHeader:
      hoist.navigationOptions = props => {
        const wrapScreenNavOptions = WrappedScreen.navigationOptions
          ...
          : {}
        const userOptions =
          typeof wrapScreenNavOptions === 'function'
            ? wrapScreenNavOptions(props)
            : wrapScreenNavOptions

        const { navigationOptions, navigation } = props
        return collapsibleNavigationOptions(
          navigationOptions,
          userOptions,
          navigation
        )
...
export const withCollapsibleForTab = (
  ...
  const hoist = hoistNonReactStatic(
    _withCollapsibleForTab,
    MaterialTopTabNavigator
  )
...
export const withCollapsibleForTabChild = WrappedScreen => {
  class _withCollapsibleForTabChild extends Component {
    render() {
      ...
      const index = this.props.screenProps.navigation.state.routes.findIndex(
        item => item.key === key
      )
      const collapsible = {
      ...
      return <WrappedScreen {...this.props} collapsible={collapsible} />
    }
  }
