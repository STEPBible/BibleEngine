/* SOURCE: https://github.com/SteffeyDev/react-native-popover-view */
'use strict';
...
type PopoverProps = {
  ...
  doneClosingCallback?: (...args: any[]) => any;
  showInModal?: boolean;
  fromRect?: {
    [key: string]: number;
  };
  fromView?: object;
  calculateRect?: (...args: any[]) => any;
  layoutRtl?: boolean;
...
type PopoverState = { fromRect: any } & { defaultDisplayArea: any } & {
  shiftedDisplayArea: null;
} & {
  shiftedDisplayArea: { x: any; y: any; width: any; height: number };
} & any &
  any & { fromRect: any } & any & { shiftedDisplayArea: null } & {
...
class Popover extends React.PureComponent<PopoverProps, PopoverState> {
  ...
  debug(line) {
    if (DEBUG || this.props.debug) console.log(line);
  }
  setDefaultDisplayArea(evt) {
    ...
    if (
      ...
      this.debug(
        'setDefaultDisplayArea - newDisplayArea: ' +
          JSON.stringify(newDisplayArea)
      );
      ...
      if (this.skipNextDefaultDisplayArea)
        this.debug(
          'setDefaultDisplayArea - Skipping first because isLandscape'
        );
    ...
    if (
      ...
      if (this.state.isAwaitingShow) {
        if (
          ...
          this.debug(
            'measureContent - Showing Popover - geom: ' + JSON.stringify(geom)
          );
  ...
  computeBottomGeometry({ displayArea, fromRect, requestedContentSize }) {
    ...
    let viewLargerThanDisplayArea = {
      height:
        ...
        Math.ceil(displayArea.y + displayArea.height),
      width: requestedContentSize.width > Math.ceil(displayArea.width)
    };
    let viewWidth = viewLargerThanDisplayArea.width
      ? forcedContentSize.width
  ...
  computeLeftGeometry({ displayArea, fromRect, requestedContentSize }) {
    ...
    let viewLargerThanDisplayArea = {
      ...
      width:
        requestedContentSize.width >
        Math.ceil(fromRect.x - displayArea.x - arrowSize.width)
    };
    let viewWidth = viewLargerThanDisplayArea.width
      ? forcedContentSize.width
      : requestedContentSize.width;
  ...
  computeAutoGeometry({ displayArea, requestedContentSize, fromRect }) {
    ...
    this.debug(
      'computeAutoGeometry - possiblePlacements: ' +
        JSON.stringify(possiblePlacements)
    );
  ...
  getCalculatedArrowDims() {
    ...
    const arrowHeight =
      (arrowStyle.height || DEFAULT_ARROW_SIZE.height) * 2 + 2;
  ...
  getArrowTranslateLocation(translatePoint = null) {
    ...
    return new Point(
      FIX_SHIFT /* Temp fix for useNativeDriver issue */ + arrowX,
      arrowY
    );
  }
  getTranslateOrigin() {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      !importantProps.reduce(
        (acc, key) => acc || this.props[key] !== nextProps[key],
        false
      )
    )
      return;
    let willBeVisible = nextProps.isVisible;
    let { isVisible, displayArea, showInModal } = this.props;
  ...
  calculateRect(props, callback) {
    ...
    if (props.calculateRect)
      runAfterChange(
        callback_ =>
          callback_(
            props.calculateRect(newDisplayArea.width, newDisplayArea.height)
          ),
        initialRect,
        () => {
          callback({
            fromRect: props.calculateRect(
              newDisplayArea.width,
              newDisplayArea.height
            )
          });
        }
      );
  ...
  handleGeomChange(requestedContentSize) {
    ...
    requestedContentSize =
      requestedContentSize ||
      Object.assign({}, this.state.requestedContentSize);
    this.debug(
      'handleGeomChange - requestedContentSize: ' +
        JSON.stringify(requestedContentSize)
    );
    ...
    if (
      ...
      this.setState(Object.assign(geom, { requestedContentSize }), () => {
        if (this.updateCount <= 1) {
          ...
          this.debug(
            'handleGeomChange - Triggering popover move to: ' +
              JSON.stringify(moveTo)
          );
          this.animateTo({
  ...
  render() {
    ...
    if (this.props.showBackground)
      backgroundStyle.backgroundColor = 'rgba(0,0,0,0.35)';
    let containerStyle = {
      ...styles.container,
