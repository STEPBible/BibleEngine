...
const DEBUG = false
const MULTIPLE_POPOVER_WARNING =
...
class Popover extends React.Component<Props, any> {
  ...
  getDisplayAreaOffset(displayArea, callback) {
    ...
    if (this.props.mode !== POPOVER_MODE.RN_MODAL) {
      getRectForRef(this.containerRef, rect =>
        callback(new Point(rect.x, rect.y + FIX_SHIFT))
      )
    } else {
  ...
  setDefaultDisplayArea(evt) {
    ...
    if (
      ...
      rectChanged(this.state.defaultDisplayArea, newDisplayArea)
    ) {
      ...
      if (!this.skipNextDefaultDisplayArea) {
        this.getDisplayAreaOffset(newDisplayArea, displayAreaOffset => {
          this.debug(
            'setDefaultDisplayArea - displayAreaOffset',
            displayAreaOffset
          )
          this.setState(
            ...
            () => {
              this.calculateRect(fromRect => {
                ...
                this.debug(
                  'setDefaultDisplayArea (inside calculateRect callback) - getDisplayArea()',
                  this.getDisplayArea()
                )
                this.debug(
                  'setDefaultDisplayArea (inside calculateRect callback) - displayAreaStore',
                  this.displayAreaStore
                )
                if (
                  rectChanged(fromRect, this.state.fromRect) ||
  ...
  measureContent(requestedContentSize) {
    ...
    if (
      ...
      !this.waitForResizeToFinish
    ) {
      if (this.state.isAwaitingShow) {
        if (
          (this.props.fromView && !this.state.fromRect) ||
          !this.getDisplayArea()
        ) {
          this.debug(
          ...
          this.debug(
            'measureContent - Showing Popover - requestedContentSize',
            requestedContentSize
          )
          ...
          if (
            geom.viewLargerThanDisplayArea.width ||
            geom.viewLargerThanDisplayArea.height
          ) {
            this.debug(
              'measureContent - Delaying showing popover because viewLargerThanDisplayArea'
            )
  ...
  computeGeometry({ requestedContentSize, placement, fromRect, displayArea }) {
    ...
    fromRect =
      fromRect || Object.assign({}, this.props.fromRect || this.state.fromRect)
    ...
    if (fromRect && isRect(fromRect)) {
      ...
      if (
        ...
        if (
          ...
          if (
            constrainedX + requestedContentSize.width >
            displayArea.x + displayArea.width
          )
            constrainedX =
              displayArea.x + displayArea.width - requestedContentSize.width

          let constrainedY = Math.max(preferedY, displayArea.y)
          if (
            constrainedY + requestedContentSize.height >
            displayArea.y + displayArea.height
          )
            constrainedY =
              displayArea.y + displayArea.height - requestedContentSize.height
    ...
    if (!newGeom) {
      ...
      const preferedY =
        (displayArea.height - requestedContentSize.height) / 2 + displayArea.y
      const preferedX =
        (displayArea.width - requestedContentSize.width) / 2 + displayArea.x

      newGeom = {
        popoverOrigin: new Point(
          Math.max(minX, preferedX),
  ...
  computeTopGeometry({ displayArea, fromRect, requestedContentSize }) {
    ...
    let viewWidth = viewLargerThanDisplayArea.width
    ...
    anchorPoint.x = Math.max(
      anchorPoint.x,
      arrowSize.width / 2 + this.getBorderRadius()
    )
  ...
  computeBottomGeometry({ displayArea, fromRect, requestedContentSize }) {
    ...
    let viewLargerThanDisplayArea = {
      height:
        preferedY + requestedContentSize.height >
        displayArea.y + displayArea.height + 1,
      width: requestedContentSize.width > displayArea.width + 1,
    }
  ...
  computeLeftGeometry({ displayArea, fromRect, requestedContentSize }) {
    ...
    let viewLargerThanDisplayArea = {
      ...
      width:
        requestedContentSize.width >
        fromRect.x - displayArea.x - arrowSize.width + 1,
    }

    let viewWidth = viewLargerThanDisplayArea.width
      ? forcedContentSize.width
      : requestedContentSize.width
    let viewHeight = viewLargerThanDisplayArea.height
    ...
    // Make sure the arrow isn't cut off
    anchorPoint.y = Math.max(
      anchorPoint.y,
      arrowSize.height / 2 + this.getBorderRadius()
    )
  ...
  computeAutoGeometry({ displayArea, requestedContentSize, fromRect }) {
    ...
    if (
      possiblePlacements.length === 2 &&
      this.state.placement !== PLACEMENT_OPTIONS.AUTO &&
      possiblePlacements.indexOf(this.state.placement) !== -1
    ) {
      let geom = this.computeGeometry({
        requestedContentSize,
    ...
    if (
      this.state.placement === PLACEMENT_OPTIONS.TOP ||
      this.state.placement === PLACEMENT_OPTIONS.BOTTOM
    )
      return this.computeGeometry({
        requestedContentSize,
        placement: this.state.placement,
  ...
  getArrowSize(placement) {
    var size = new Size(
      this.props.arrowStyle.width || DEFAULT_ARROW_SIZE.width,
      this.props.arrowStyle.height || DEFAULT_ARROW_SIZE.height
    )
    switch (placement) {
      case PLACEMENT_OPTIONS.LEFT:
      case PLACEMENT_OPTIONS.RIGHT:
    ...
    if (translatePoint) {
      if (
        ...
        if (translatePoint.y > arrowY - this.getBorderRadius())
          ...
          viewHeight &&
          translatePoint.y + viewHeight < arrowY + arrowHeight
        )
          arrowY =
            translatePoint.y + viewHeight - arrowHeight - this.getBorderRadius()
      } else if (
        placement === PLACEMENT_OPTIONS.TOP ||
        placement === PLACEMENT_OPTIONS.BOTTOM
      ) {
        if (translatePoint.x > arrowX - this.getBorderRadius())
          ...
          viewWidth &&
          translatePoint.x + viewWidth < arrowX + arrowWidth
        )
          arrowX =
            translatePoint.x + viewWidth - arrowWidth - this.getBorderRadius()
      }
    }
    return new Point(
      arrowX,
      FIX_SHIFT * 2 /* Temp fix for useNativeDriver issue */ + arrowY
    )
  }
  ...
  getTranslateOrigin() {
    ...
    const popoverWidth = viewLargerThanDisplayArea.width
      ? forcedContentSize.width
      : requestedContentSize.width || 0
    const popoverHeight = viewLargerThanDisplayArea.height
      ? forcedContentSize.height
      : requestedContentSize.height || 0
    const popoverCenter = new Point(
      popoverOrigin.x + popoverWidth / 2,
      popoverOrigin.y + popoverHeight / 2
    )
    ...
    return new Point(
      popoverOrigin.x + shiftHorizantal,
      popoverOrigin.y + shiftVertical
    )
  }

  getDisplayArea() {
    return (
      this.state.shiftedDisplayArea ||
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      !importantProps.reduce(
        (acc, key) => acc || this.props[key] !== prevProps[key],
        false
      )
    )
      return
  ...
  calculateRect(callback) {
    ...
    if (this.props.fromDynamicRect)
      runAfterChange(
        callback_ =>
          callback_(
            this.props.fromDynamicRect(displayArea.width, displayArea.height)
          ),
        initialRect,
        () => {
          callback({
            fromRect: this.props.fromDynamicRect(
              displayArea.width,
              displayArea.height
            ),
          })
      ...
      const verticalOffset =
        ...
        (this.state.displayAreaOffset ? -1 * this.state.displayAreaOffset.y : 0)
      const horizontalOffset = this.state.displayAreaOffset
        ? -1 * this.state.displayAreaOffset.x
        : 0
  ...
  handleGeomChange(requestedContentSize) {
    return // NOTE: temporarily disabling this function, as it causes incorrect position changes on Android
    const { forcedContentSize, popoverOrigin, animatedValues } = this.state
    requestedContentSize =
      requestedContentSize || Object.assign({}, this.state.requestedContentSize)
    ...
    if (
      pointChanged(geom.popoverOrigin, popoverOrigin) ||
      rectChanged(geom.forcedContentSize, forcedContentSize)
    ) {
      this.setState(Object.assign(geom, { requestedContentSize }), () => {
  ...
  animateOut() {
    ...
    this.animateTo({
      ...
      callback: () =>
        this.setState({ visible: false, forcedContentSize: {} }, () => {
  ...
  animateIn() {
    ...
    const translatePoint = new Point(
      this.state.popoverOrigin.x,
      this.state.popoverOrigin.y
    )
    values.translateArrow.setValue(
      this.getArrowTranslateLocation(translatePoint)
    )
    ...
    this.animateTo({
      ...
      callback: () => {
        ...
        setTimeout(() =>
          getRectForRef(this.popoverRef, rect =>
            this.debug(
              'animateIn - onOpenComplete - Calculated Popover Rect',
              rect
            )
          )
        )
  ...
  render() {
    ...
    let arrowInnerStyle = [
      ...
      this.getArrowDynamicStyle(),
      {
        ...
        transform: [{ rotate: this.getArrowRotation(placement) }],
      },
    ]
        ...
        <Animated.View pointerEvents="box-none" style={containerStyle}>
          ...
          <View pointerEvents="box-none" style={{ top: 0, left: 0 }}>
            <Animated.View
              ...
              onLayout={evt => this.measureContent(evt.nativeEvent.layout)}
            >
              {this.props.children}
            </Animated.View>
...
Popover.propTypes = {
  ...
  mode: PropTypes.oneOf([
    POPOVER_MODE.JS_MODAL,
    POPOVER_MODE.RN_MODAL,
    POPOVER_MODE.TOOLTIP,
  ]),
  ...
  placement: PropTypes.oneOf([
    PLACEMENT_OPTIONS.LEFT,
    PLACEMENT_OPTIONS.RIGHT,
    PLACEMENT_OPTIONS.TOP,
    PLACEMENT_OPTIONS.BOTTOM,
    PLACEMENT_OPTIONS.AUTO,
  ]),
